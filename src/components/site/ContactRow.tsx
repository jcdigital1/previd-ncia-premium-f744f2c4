...
export function ContactRow({ compact }: { compact?: boolean }) {
  return (
    <div className="mx-auto w-full max-w-lg">
      ...
      <div className={`grid grid-cols-4 gap-2 ${compact ? "mt-0" : "mt-3"}`}>
        {contacts.map(({ label, href, Icon }) => (
          <a
            ...
            className={`flex flex-col items-center gap-1.5 rounded-2xl border border-border bg-card/60 px-2 backdrop-blur-sm transition-all duration-300 hover:border-gold/50 hover:bg-card active:scale-[0.97] ${compact ? "py-2" : "py-3"}`}
          >
